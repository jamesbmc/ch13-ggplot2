...
all.data <- mutate(all.data, percent.voted = total.voted / registered.voters)
...
ggplot(data = all.data) +
  geom_text(mapping = aes(x = total.population, y = percent.voted, label = county))

# Draw a scatter plot comparing the female percentage to the percent voted
# Size the points by the population of the county
ggplot(data = all.data) +
  geom_point(mapping = aes(x = female.percentage, y = percent.voted, size = total.population))

...
county.elections <- mutate(county.elections, dem.winner = votes_dem > votes_gop)
dem.won <- data.frame(dem.winner = c(T, F), party = c("Democrat", "Republican"))
county.elections <- left_join(county.elections, dem.won, by = "dem.winner")
